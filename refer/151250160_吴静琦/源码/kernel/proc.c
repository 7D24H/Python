...
{
	//Orange's源码，根据不同进程对应的tick或priority调度，tick值大的优先执行
	
	struct proc*	p;
	int		greatest_ticks = 0;
	while (!greatest_ticks) {
		//下面这个for循环决定取哪一个作为下一个执行的进程
		//ticks值最大的进程是下一个执行的进程，并把greatest_ticks的值设为这个进程ticks的值
		...
		//如果所有的进程ticks都为0，那就用priority的值去调度		
		if (!greatest_ticks){
			for (p = &FIRST_PROC; p <= &LAST_PROC; p++){
				if (p->p_flags == 0){
					p->ticks = p->priority;
 ...
 *                                scheduleWithSleep
 *****************************************************************************/
//为了完成实验，额外增加“对于特定的进程，一段时间内不分配时间片”功能
