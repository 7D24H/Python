...
void searchContentsInSpecificPath(char* order);
...
int main(){
	...
	while(1){
		...
		//详细文件内容输出
			...
			//scanf("%s",path);
			//asmPrint(path,strlen(path));
			searchContentsInSpecificPath(order);
		}
...
void lsAll(FILE* fat12){
	...
	int offset=0x00;
	//while(&(ROOT_START+offset)!=0){
	showNames(fat12, "", ROOT_START, dir1);
	asmPrint("\n",1);
	showNames(fat12, "", DATA_START+0x240, dir1);
	asmPrint("\n",1);
	showNames(fat12, "", DATA_START+0x440, dir1);
	//offset+=0x40;
	//}
...
//	asmPrint("\n",1);
//	showNames(fat12,"",DATA_START+0x240,dir1);
//	showNames(fat12,"",DATA_START+0x280,dir1);
//	showNames(fat12,"",DATA_START+0x2C0,dir1);
//	showNames(fat12,"",DATA_START+0x300,dir1);
...
void searchContentsInSpecificPath(char* order){
		...
		//	}
			...
			if(strcmp(order, "count") == 0){			//inputStrPtr->order	
				scanf("%s",inputStrPtr);						
				mode = COUNT_MODE;
			}
			else{			
...
void printWarning(char* warning){
	prepareWarning();
	asmPrint(warning,strlen(warning));	
	//printf("%s",&warning);
	prepareTerminal();
}
